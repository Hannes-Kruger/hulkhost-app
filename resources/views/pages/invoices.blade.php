<div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
    ...
    <div class="flex mx-auto">
        <div>
            ...
            @else
                <flux:subheading size="lg" class="mb-6">{{ __('You have no outstanding invoices.') }}</flux:subheading>
            @endif
        </div>

        <flux:spacer/>

        <flux:button variant="primary" href="{{ route('payment-details') }}"
                     wire:navigate>{{ __('Payment Instructions') }}</flux:button>
    ...
    <flux:table>
        <flux:columns>
            <flux:column  sortable :sorted="$sortBy === 'DocumentNumber'" :direction="$sortDirection"
                         wire:click="sort('DocumentNumber')">ID
            </flux:column>
        ...
        <flux:rows>
            ...
            @foreach($this->invoices as $invoice)
                <flux:row class="hover:dark:bg-black/20 hover:bg-gray-200">
                    ...
                    <flux:cell>
                        <flux:badge color="{{ data_get($invoice, 'BadgeColor') }}" size="sm"
                                    inset="top bottom">{{ data_get($invoice, 'Status') }}</flux:badge>
                    </flux:cell>
            ...
            @if($this->invoices->isEmpty())
                <flux:row>
                    <flux:cell colspan="6" class="text-center">{{ __('This account has no invoices yet.') }}</flux:cell>
                </flux:row>
            @endif
        </flux:rows>
    </flux:table>
